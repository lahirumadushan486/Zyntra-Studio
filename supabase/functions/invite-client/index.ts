...
Deno.serve(async (request) => {
  ...
  const inviteResult = await adminClient.auth.admin.inviteUserByEmail(email, {
    ...
    data: { full_name: fullName, business_name: businessName, phone, requested_client_id: clientLoginId || null }
  });
  if (inviteResult.error || !inviteResult.data.user) {
    const exists = /already|registered|exists/i.test(inviteResult.error?.message || '');
    const idUnavailable = Boolean(clientLoginId) && /database|duplicate|unique|client id/i.test(inviteResult.error?.message || '');
    return response(origin, exists || idUnavailable ? 409 : 500, { success: false, error: idUnavailable ? 'That Client ID is unavailable.' : exists ? 'A user with this email already exists.' : 'The client invitation could not be sent.' });
  }

  const invitedUser = inviteResult.data.user;
  const existingProfile = await adminClient.from('profiles').select('role').eq('id', invitedUser.id).maybeSingle();
  if (existingProfile.error) return response(origin, 500, { success: false, error: 'The invitation was sent, but the client profile could not be verified.' });
  if (existingProfile.data && String(existingProfile.data.role).trim().toLowerCase() === 'admin') {
    return response(origin, 409, { success: false, error: 'An administrator profile cannot be replaced.' });
  }

  const safeProfile = { full_name: fullName, business_name: businessName, phone: phone || null, active: true };
  const profileWrite = existingProfile.data
    ? await adminClient.from('profiles').update(safeProfile).eq('id', invitedUser.id).eq('role', 'client').select('id,role').single()
    : await adminClient.from('profiles').insert({ id: invitedUser.id, ...safeProfile, role: 'client' }).select('id,role').single();
  if (profileWrite.error || profileWrite.data?.role !== 'client') return response(origin, 500, { success: false, error: 'The invitation was sent, but the client profile could not be prepared.' });
